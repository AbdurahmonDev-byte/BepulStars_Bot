...
class StarsRecipientStates(StatesGroup):
    """Yulduz (⭐) yechishda 'boshqa odamga' yuborish tanlanganda, qabul
    qiluvchining @username'ini kutish holati — xuddi GiftRecipientStates
    kabi, lekin ⭐ yechish uchun (mijoz talabi keyinroq kengaytirildi)."""
    username = State()
...
async def db_init() -> None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for alter_sql in (
            ...
            "ALTER TABLE settings ADD COLUMN refs_per_spin INTEGER NOT NULL DEFAULT 2",
        ):
            try:
        ...
        await db.execute("""
            ...
            VALUES
                ...
                ('spin', '🎡 Jekpot aylantirish', 0, 5, 20, 0.15, 1, 'gift', 0, 'Referal evaziga erkin aylantirish — yutuq: 5–20 ⭐ yoki gift')
        """)
...
async def add_user(telegram_id: int, referrer_id: int | None = None, phone_number: str = "") -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT INTO users (telegram_id, referrer_id, phone_number, joined_at) VALUES (?, ?, ?, ?)",
            (telegram_id, referrer_id, phone_number, datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        )
        await db.commit()


def normalize_phone(raw: str) -> str:
    """Telefon raqamni yagona ko'rinishga keltiradi: probel, tire, qavs va
    '+' belgilari olib tashlanadi (raqamlar qoladi). Shu tufayli
    +998 90 123 45 67 / 998901234567 / +998901234567 bir xil hisoblanadi."""
    return "".join(ch for ch in raw if ch.isdigit())


async def set_user_phone(telegram_id: int, phone: str) -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET phone_number = ? WHERE telegram_id = ?",
            (normalize_phone(phone), telegram_id),
        )
        await db.commit()
...
async def reset_all_users_balance() -> int:
    """Barcha foydalanuvchilarning ⭐ balansini 0 ga tushiradi.
    O'zgargan foydalanuvchilar sonini qaytaradi."""
    async with aiosqlite.connect(DB_PATH) as db:
...
async def register_user_with_referral(telegram_id: int, referrer_id: int | None, friend_name: str = "", phone_number: str = "") -> dict:
    """Yangi foydalanuvchini ro'yxatga oladi va referal mukofotini to'laydi."""
    user = await get_user(telegram_id)
    if user:
        if phone_number and not user["phone_number"]:
            await set_user_phone(telegram_id, phone_number)
        return user, False
...
async def bot_notify_referrer(referrer_id: int, friend_name: str, refs_per_spin: int, ref_count: int) -> None:
    ...
    Referal ⭐ bermaydi — har refs_per_spin referalga bitta 🎡 jekpot
    aylantirish yig'iladi. Aylantirish sovrin o'tganida alohida ta'kidlanadi."""
    if _bot is None:
        return
    if refs_per_spin > 0 and ref_count % refs_per_spin == 0:
        ...
        text = (
            ...
            f"🎡 Aylantirish uchun yana <b>{refs_per_spin - (ref_count % refs_per_spin)}</b> referal kerak ({ref_count % refs_per_spin}/{refs_per_spin})"
        )
    try:
...
async def _fulfill_stars_withdrawal(
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                ...
                f"💰 Miqdor: <b>{amount} ⭐</b>\n"
                f"📅 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                f"⚠️ {'YUQORIDAGI QABUL QILUVCHIGA' if recipient_id else 'Foydalanuvchiga'} real to'lovni "
                f"o'tkazgach, <b>\"✅ To'lov qildim\"</b> tugmasini bosing — "
    ...
    if recipient_id:
        return (
            ...
            f"💰 Miqdor: <b>{amount} ⭐</b>\n"
            f"🧾 So'rov: #{w_id}\n\n"
            f"📤 Yulduzlar <b>{recipient_display}</b>ga bot egasi tomonidan o'tkaziladi.\n"
...
async def process_stars_withdrawal(call: CallbackQuery, bot: Bot, amount: int) -> None:
    """"🙋 O'zimga" — yulduzlar (haqiqiy to'lov sifatida) so'rovchining
    o'ziga o'tkaziladi. Balansdan ayriladi, admin uchun Tasdiqlash/Rad etish
    tugmali so'rov yaratiladi (holati kuzatiladi — ikki marta to'lanib
    ketmasin)."""
    user = await get_user(call.from_user.id)
    if not user:
...
async def withdraw_stars_callback(call: CallbackQuery) -> None:
    ...
    "kimga?" so'raladi — o'ziga yoki boshqa odamga (@username orqali)."""
    user = await get_user(call.from_user.id)
    if not user:
        await call.answer("❌ Avval /start ni bosing!", show_alert=True)
        return
...
@router.callback_query(F.data == "starsrecipient:self")
async def stars_recipient_self(call: CallbackQuery, bot: Bot) -> None:
    """"🙋 O'zimga" tanlandi — avvalgi (yagona) xatti-harakat."""
    user = await get_user(call.from_user.id)
    if not user:
...
@router.callback_query(F.data == "starsrecipient:other")
async def stars_recipient_other_start(call: CallbackQuery, state: FSMContext) -> None:
...
@router.message(StarsRecipientStates.username)
async def stars_recipient_other_finish(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    if not await deduct_stars(message.from_user.id, amount):
...
async def show_boxes(answer_func, telegram_id: int, result_text: str | None = None) -> None:
    ...
    user = await get_user(telegram_id)
    today = datetime.now().strftime("%Y-%m-%d")
    boxes = [b for b in await get_all_boxes() if b["box_id"] != "spin"]
    ...
    if user and user["free_spins"] > 0:
        kb.button(text=f"🎡 Aylantirish (bepul — {user['free_spins']} ta)", callback_data="jeckpot_spin")
    ...
    if user:
        settings = await get_settings()
        refs_per_spin = settings.get("refs_per_spin") or 0
        text += f"🎡 <b>Jekpot aylantirishlar: {user['free_spins']}</b>"
        if refs_per_spin > 0:
            text += f" (har {refs_per_spin} referal uchun +1)"
        text += "\n\n"
...
async def _get_spin_box() -> dict:
    """Referal evaziga erkin aylantirish uchun ishlatiladigan box."""
    box = await get_box("spin")
    if box:
        return box
    return {
        "box_id": "spin",
        "name": "🎡 Jekpot aylantirish",
        "cost": 0,
        "star_min": 5,
        "star_max": 20,
        "gift_drop_prob": 0.15,
        ...
        "gift_category": "gift",
        "once_per_day": 0,
        "desc_text": "Referal evaziga erkin aylantirish",
    }


@router.callback_query(F.data == "jeckpot_spin")
async def jeckpot_spin_callback(call: CallbackQuery, bot: Bot) -> None:
    """Erkin (referal evaziga) jekpot aylantirish — hech narsa ayirilmaydi,
    faqat free_spins hisobidan bittasi konsum qilinadi."""
    user = await get_user(call.from_user.id)
    if not user:
        await call.answer("❌ Avval /start ni bosing!", show_alert=True)
        return
    ...
    if not await consume_free_spin(call.from_user.id):
        await call.answer("❌ Erkin aylantirish qolmadi!", show_alert=True)
        return

    result = await open_box_and_award(
        bot, await _get_spin_box(), call.from_user.id, call.from_user.first_name, call.from_user.username,
    )

    await call.answer("🎡 Aylantirildi!", show_alert=False)
    try:
        await call.message.delete()
    except TelegramBadRequest:
        pass
...
def admin_keyboard() -> InlineKeyboardMarkup:
    ...
    kb.button(text="🔄 Barchani balansini 0 qilish", callback_data="admin:reset_all_balances:confirm")
    kb.adjust(2)
    return kb.as_markup()
...
@router.callback_query(F.data == "admin:reset_all_balances:confirm")
async def admin_reset_all_balances_confirm(call: CallbackQuery) -> None:
    if not is_admin(call.from_user.id):
        await call.answer("❌ Siz admin emassiz!", show_alert=True)
        return
    kb = InlineKeyboardBuilder()
    kb.button(text="⚠️ HA, BARCHASINI 0 QILISH", callback_data="admin:reset_all_balances:execute")
    kb.button(text="❌ Bekor qilish", callback_data="admin")
    kb.adjust(1)
    await call.message.edit_text(
        "⚠️ <b>DIQQAT! Barcha foydalanuvchilar balansini 0 ga tushirish</b>\n\n"
        "Rostdan ham bazadagi <b>BARCHA foydalanuvchilarning ⭐ balansini 0 ga</b> tushirmoqchimisiz?\n\n"
...
@router.callback_query(F.data == "admin:reset_all_balances:execute")
async def admin_reset_all_balances_execute(call: CallbackQuery) -> None:
    if not is_admin(call.from_user.id):
        await call.answer("❌ Siz admin emassiz!", show_alert=True)
        return
    count = await reset_all_users_balance()
    await call.answer("✅ Barcha foydalanuvchilar balansi 0 ga tushirildi!", show_alert=True)
    await call.message.edit_text(
        f"✅ <b>Muvaffaqiyatli bajarildi!</b>\n\n"
        f"Jami <b>{count} ta</b> foydalanuvchining balansi <b>0 ⭐</b> ga tushirildi.",
...
async def ref_spin_input(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(
        f"✅ Aylantirish holati qo'yildi: har <b>{value}</b> referalga 1 ta 🎡"
        + (" (0 — aylantirish berilmaydi)" if value == 0 else ""),
        reply_markup=admin_keyboard(),
    )
...
function spinCard() {
  const card = document.createElement('div');
  card.className = 'card jackpot spin-card';
  const spins = (USER && USER.free_spins) || 0;
  const refsPer = (SHOP.settings && SHOP.settings.refs_per_spin) || 2;
  card.innerHTML = `
    <div class="icon-tile">🎡</div>
    <div class="name">Jekpot aylantirish</div>
    <div class="desc">Referal evaziga yig'ilgan erkin aylantirish. Haydang — raketa uchadi!</div>
    <div class="price">${spins} ta erkin aylantirish</div>
    <div class="sub">har ${refsPer} referal uchun +1</div>
    <div class="btnrow">
      <button data-act="spin">🎡 Aylantirish</button>
    </div>
  `;
  const btn = card.querySelector('button');
  if (!(spins > 0)) btn.disabled = true;
  btn.onclick = () => spinNow(btn);
  return card;
}

async function spinNow(btnEl) {
  if (!INIT_DATA) { toast('Bu amal uchun botni Telegram ilovasi ichidan oching'); return; }
  btnEl.disabled = true;
  const oldText = btnEl.textContent;
  btnEl.textContent = 'Aylantirilmoqda...';
  try {
...
function renderWithdraw() {
  ...
  gifts.forEach(g => {
    ...
    row.querySelector('button').onclick = () => openRecipientChoice('gift', g.id, `🎁 ${g.name}`);
    list.appendChild(row);
  });
}

document.getElementById('wdStarsBtn').onclick = () => openRecipientChoice('stars', null, `⭐ ${USER ? USER.balance_stars : 0} yulduz`);

// ---- Yechishda "kimga?" tanlovi (mijoz talabi: gift HAM, yulduz HAM
// faqat o'ziga emas, boshqa odamga ham yuborilishi kerak) ----
let pendingKind = null;
let pendingItemId = null;
function openRecipientChoice(kind, itemId, label) {
  pendingKind = kind;
...
async def webapp_spin_handler(request):
    """Mini App'dagi 🎰 Jekpot tabidan erkin aylantirish — referal evaziga
    yig'ilgan free_spins'ni konsum qilib, box yechiladi va natija qaytariladi.
    Webapp shu javob bilan raketani uchirib, natijani ko'rsatadi."""
    if _bot is None:
        return web.json_response({"error": "Bot hali tayyor emas"}, status=503)
    ...
    if not await consume_free_spin(telegram_id):
        return web.json_response({"error": "Erkin aylantirish qolmadi! Referal taklif qilib yig'ing."}, status=400)

    result = await open_box_and_award(
        _bot, await _get_spin_box(), telegram_id,
        tg_user.get("first_name", ""), tg_user.get("username", ""),
    )
    has_auto_delivery = await _gift_claim_has_auto_delivery(result["claim_id"]) if result["claim_id"] else False
    db_user = await get_user(telegram_id)
    return web.json_response({
...
def register_webapp_routes(app: "web.Application") -> None:
    ...
    app.router.add_post("/api/spin", webapp_spin_handler)
